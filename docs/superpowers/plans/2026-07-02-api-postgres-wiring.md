...
**Goal:** Make `cmd/megagega-api` bootstrap the Postgres store from `DATABASE_URL`.

**Architecture:** `internal/config` owns environment loading and validation. `cmd/megagega-api` owns process composition: config, pgx pool, migrations, store construction, and `app.NewService` wiring. Startup remains short-lived until the HTTP server slice exists.
...
- Create: `cmd/megagega-api/main_test.go`
...
Run without DSN: `go test ./cmd/megagega-api`

Expected: PASS with integration test skipped.

Run with DSN: `MEGAGEGA_POSTGRES_TEST_DSN='postgres://megagega:megagega@localhost:55432/megagega_test?sslmode=disable' go test -count=1 ./cmd/megagega-api`
...
Run: `MEGAGEGA_POSTGRES_TEST_DSN='postgres://megagega:megagega@localhost:55432/megagega_test?sslmode=disable' go test -count=1 ./...`
...
```bash
git add docs/superpowers/specs/2026-07-02-api-postgres-wiring-design.md docs/superpowers/plans/2026-07-02-api-postgres-wiring.md internal/config/config.go internal/config/config_test.go cmd/megagega-api/main.go cmd/megagega-api/main_test.go
