...
func (store *Store) ReconcileTemplateRunCancellation(ctx context.Context, tenantID domain.TenantID, runID domain.TemplateRunID, errorSummary string) error {
	tx, err := store.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin reconcile template run cancellation: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	...
	if err := releaseRunPlan(ctx, tx, tenantID, runID); err != nil {
		return err
	}
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit reconcile template run cancellation: %w", err)
	}
	return nil
}
